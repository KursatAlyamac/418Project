...
class Planner(torch.nn.Module):
    def __init__(self):
        ...
        # New obstacle avoidance head
        self.obstacle_conv1 = torch.nn.Conv2d(128, 64, kernel_size=3, padding=1)
        self.obstacle_bn1 = torch.nn.BatchNorm2d(64)
        self.obstacle_conv2 = torch.nn.Conv2d(64, 32, kernel_size=3, padding=1)
        self.obstacle_bn2 = torch.nn.BatchNorm2d(32)
        self.obstacle_classifier = torch.nn.Conv2d(32, 1, kernel_size=1)
    ...
    def forward(self, img, return_powerup=False):
        # Enhanced encoder with batch norm and residual connections
        ...
        # Obstacle detection
        obstacle = self.relu(self.obstacle_bn1(self.obstacle_conv1(x)))
        obstacle = self.dropout(obstacle)
        obstacle = self.relu(self.obstacle_bn2(self.obstacle_conv2(obstacle)))
        obstacle = self.dropout(obstacle)
        obstacle = self.obstacle_classifier(obstacle)
        
        # Combine path planning with obstacle avoidance
        combined_features = path * (1 - torch.sigmoid(obstacle))
        aim_point = spatial_argmax(combined_features[:, 0])
