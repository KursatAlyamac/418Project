...
def train(args):
    ...
    loss_powerup = torch.nn.BCEWithLogitsLoss(pos_weight=torch.tensor([2.0]).to(device))
    loss_racing = torch.nn.MSELoss()
    ...
    with open('training_losses.txt', 'w') as f:
        ...
        for epoch in range(args.num_epoch):
            ...
            for img, label in train_data:
                ...
                aim_point, powerup_pred, racing_features = model(img, return_powerup=True)
                
                # Compute path loss
                path_loss = loss_path(aim_point, label)
                
                # Add racing line loss if available in labels
                if hasattr(label, 'racing_line'):
                    racing_loss = loss_racing(racing_features, label.racing_line)
                    racing_losses.append(racing_loss.item())
                    total_loss = path_loss + 0.5 * racing_loss
                else:
                    total_loss = path_loss
                
                # Add powerup loss if available
                if hasattr(label, 'powerup'):
                    powerup_loss = loss_powerup(powerup_pred, label.powerup)
                    powerup_weight = min(0.5, 0.1 + epoch * 0.02)
                    total_loss += powerup_weight * powerup_loss
                    powerup_losses.append(powerup_loss.item())
                ...
                path_losses.append(path_loss.item())
                
                if train_logger is not None:
                    train_logger.add_scalar('loss/path', path_loss.item(), global_step)
                    if len(powerup_losses) > 0:
                        train_logger.add_scalar('loss/powerup', powerup_losses[-1], global_step)
                
                global_step += 1
            
            epoch_time = time.time() - epoch_start_time
            avg_loss = np.mean(path_losses)
