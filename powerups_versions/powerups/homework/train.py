    ...
    with open('training_losses.txt', 'w') as f:
        for epoch in range(args.num_epoch):
            ...
            for img, label in train_data:
                ...
                # Compute path planning loss
                loss_val = loss_path(aim_point, label)
                
                # Add powerup detection loss if available in your dataset
                if hasattr(label, 'powerup'):
                    powerup_loss = loss_powerup(powerup_pred, label.powerup)
                    loss_val += 0.2 * powerup_loss
                
                if train_logger is not None:
                    train_logger.add_scalar('loss', loss_val, global_step)
                    if global_step % 100 == 0:
                        log(train_logger, img, label, aim_point, global_step)
